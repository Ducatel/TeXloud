		...
		$sql = $bdd->prepare('SELECT idutilisateur FROM utilisateur WHERE  identifiant= ?');
           ...
           /* Effacement de toutes les lignes de la table FRUIT */
              ...
              if($sql == 0){
			    ...
			    $motpassedeux = sha1($_POST['motpassedeux']);
			      ...
			      $sql=$bdd->prepare('INSERT INTO utilisateur (identifiant,motpasse,motpassedeux,mail) VALUES  (?,?,?,?)');
			      $sql->execute(array($_POST['identifiant'],$motpasse,$motpassedeux,$_POST['mail']));
